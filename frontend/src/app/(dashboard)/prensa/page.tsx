...
export default function PrensaPage() {
  const { data: personal = [], isLoading, isError } = usePersonal('PRENSA');
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Acredita y gestiona a los periodistas, fotógrafos y medios de comunicación.
        </p>
      </div>
          ...
          {personal.length > 0 && (
            <span>
              {filtrado.length !== personal.length
                ...
                : <><span className="font-medium text-slate-700">{personal.length}</span> personas acreditadas</>}
            </span>
          )}
        </p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-48">
            <label className="block text-xs font-medium text-slate-500 mb-1.5">Buscar (Nombre o CC)</label>
          ...
          <div className="w-36">
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500"
            >
              <option value="todos">Todos</option>
              <option value="activo">Acreditado</option>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-red-600">Error cargando medios.</div>
        ) : personal.length === 0 ? (
          <div className="p-16 text-center text-slate-500">No hay periodistas registrados aún.</div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left text-slate-600">
                <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
                  <tr>
                    <th className="px-6 py-4 font-medium">Periodista / Medio</th>
                    <th className="px-6 py-4 font-medium">Documento</th>
                    <th className="px-6 py-4 font-medium">Rol (Credencial)</th>
                    <th className="px-6 py-4 font-medium">Acreditación</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {paginated.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-xs font-semibold">{p.cargoNombre || 'Prensa'}</span>
                      </td>
                      <td className="px-6 py-4">
                        {p.activo
                          ? <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-md text-xs font-medium border border-blue-200">Aprobado</span>
                          : <span className="px-2 py-1 bg-slate-100 text-slate-500 rounded-md text-xs font-medium border border-slate-200">Revocado</span>}
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-1">
                          ...
                          <button onClick={() => setCarnetPersonal(p)} title="Ver Credencial"
                            className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors">
                          ...
                          {p.activo && (
                            <button onClick={() => desactivarMutation.mutate(p.id)} title="Revocar Credencial"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="bg-white rounded-xl shadow-xl w-80 p-6 relative z-10 animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-slate-800">QR de Acceso Prensa</h2>
...
// ─────────────────────────────────────────
// Modal: Registrar Prensa (3 tabs como Logística, sin camiseta)
// ─────────────────────────────────────────
const INITIAL_PRENSA: Partial<CreatePersonalDTO> = {
  primerNombre: '', primerApellido: '', tipoDocumento: 'CEDULA_CIUDADANIA', numeroDocumento: '',
  tipoPersonal: 'PRENSA', cargoId: '' as any,
...
function CreatePersonalModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [form, setForm] = useState(INITIAL_PRENSA);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate(
      ...
      { onSuccess: () => { setForm(INITIAL_PRENSA); setTab('identity'); onClose(); } }
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6 relative z-10 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-semibold text-slate-800 mb-4">Acreditar Nuevo Periodista/Medio</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {tab === 'identity' && (
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-1">Cargo / Medio (Rol) *</label>
                <select required className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm bg-white"
                  ...
                  <option value="">Seleccione tipo de acreditación...</option>
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-slate-100 mt-6">
            ...
            <button type="submit" disabled={createMutation.isPending}
              ...
              {createMutation.isPending ? 'Guardando...' : 'Acreditar'}
            </button>
          </div>
        </form>
      </div>
...
function EditPersonalModal({ personal, onClose }: { personal: Personal | null; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={onClose} />
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6 relative z-10 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-semibold text-slate-800 mb-4">Editar Personal Prensa</h2>
